...
export const addOrdersService = async (
  ...
  const resolvedPaymentMethod = options.paymentMethod || paymentMode;
  const paymentStatus =
    options.paymentStatus || (resolvedPaymentMethod === "STRIPE" ? "Paid" : "Pending");
  const subtotal = Number(totalAmount) - Number(totalShippingFee);
  let createdOrder = null;

  try {
    await db.query("BEGIN");

    const existingOrder = await db.query("SELECT order_id FROM orders WHERE session_id = $1", [
      sessionId,
    ]);
    ...
    for (const item of Object.values(cartData)) {
      await db.query(
        ...
        `,
        [orderId, item.product_id, item.quantity, item.size, item.product_price]
      );
    ...
    if (options.sendConfirmationEmail !== false && createdOrder?.email) {
      try {
